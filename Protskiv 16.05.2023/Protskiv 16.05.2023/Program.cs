...
namespace Program
{
    public static class Program
    {
        ...
        {
            void OnAgeGreaterThirty(object sender, SalaryEventArgs e);
        }
        public class UnderThirtyObserver : IAgeObserver
        {
            private List<SalaryEventArgs> salaryEvents = new List<SalaryEventArgs>();

            public void OnAgeGreaterThirty(object sender, SalaryEventArgs args)
            {
                if (args.Age < 30)
                {
            ...
            public void ReadData(string path)
            {
                ...
                foreach (var item in query)
                {
                    if (item.Salary >= 10000)
                    {
                        ...
                        {
                            observer.OnAgeGreaterThirty(this, args);
                        }
                    }
                }
            }
        ...
        public static void Main(string[] args)
        {
            ...
            //Написати кілька юніт-тестів до цього.
